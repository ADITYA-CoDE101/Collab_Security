...
both the server (s1.py, initialize.py) and all clients (c1.py).
...
-----------------------
Using these instead of hardcoded strings so typos trigger a NameError, not
...
# Server → Client  :  initial handshake / welcome (TCP) under SSL/TLS handshake 
EVT_CONNECT    = "connect"
...
def build_packet(event: str, payload: dict) -> bytes:   # Returning the  bytes since the json will be dumped into bytes.
    # Preparing the packet to send out the data.
    """
    Serialise an event + payload dict into UTF-8 JSON bytes ready to
    send over a socket.

    Usage ─> Standered json Packet.
    -----
...
def parse_packet(data: bytes) -> dict | None:   # Returning as a dictionary if the data exit else None
    # Extracting the JSON data received from the client.
    """
    ...
    if not data:
        return None
    try:
        obj = json.loads(data.decode("utf-8", errors="replace"))    #JSON object
